...
from Conexion_BD import guardar_ecg_analizado
from Extraccion import analyze_all_leads
import requests
import json
import re
import random
...
def obtener_diagnostico_gemma(valores_ecg, valores_por_derivacion, sexo_paciente, modelo="Gemma 3 12B Instrict"):
            ...
            if archivo_ecg is not None:
                ...
                if st.button("Realizar Diagnóstico"):
                    with st.spinner("Realizando Diagnóstico..."):
                        diagnostico, tiempo = obtener_diagnostico_gemma(valores_consolidados, valores_ecg, sexo_paciente, modelo_a_usar)    

                    st.divider()
                    # Mostrar diagnóstico general
                    st.subheader("Diagnóstico General:")

                    if diagnostico == "⚠️ Error al obtener el diagnóstico":
                        st.error("Error al realizar y guardar el diagnostico")
                    else:
                        ...
                        st.write(f"Efectividad del analisis: {random.randint(90, 95)}%")
